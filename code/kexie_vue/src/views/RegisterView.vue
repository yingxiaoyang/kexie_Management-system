  ...
  <main class="login-page">
    <section class="login-card register-card">
      <div class="login-intro">
        ...
        <el-alert title="学号将作为登录账号，注册后不可自行更换，请确认无误。" type="warning" :closable="false" show-icon />
      </div>
...
const form = reactive({ name: '', studentNo: '', college: '', phone: '', email: '', password: '', confirmPassword: '' })
const rules = {
...
async function submit() {
  ...
  try {
    await apiRequest('/auth/register', { method: 'POST', body: { name: form.name, studentNo: form.studentNo, college: form.college, phone: form.phone, email: form.email, password: form.password } })
    ElMessage.success('注册成功，请使用学号登录')
    await router.replace('/login')
  } catch (error) { errorMessage.value = ['DUPLICATE_RESOURCE', 'STUDENT_NUMBER_REGISTERED'].includes(error.code) ? '该学号已注册账号，请直接登录或联系管理员' : error.message }
