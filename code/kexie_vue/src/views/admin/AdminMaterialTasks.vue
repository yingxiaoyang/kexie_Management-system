  ...
  <div>
    <div class="page-header">
      <div>
        ...
        <p class="page-desc">先创建统筹任务文件夹，再在其中配置每次只接收一个文件的子任务。</p>
      </div>
    ...
    <div class="panel">
      ...
      <el-table v-loading="loading" :data="tasks" class="desktop-table" style="width: 100%" empty-text="暂无材料任务">
        <el-table-column prop="taskName" label="统筹任务" min-width="180" />
      ...
      <div class="mobile-card-list">
        ...
        <article v-for="row in tasks" :key="row.id" class="mobile-data-card">
          ...
          <div class="mobile-card-body">
            <div style="grid-column: 1 / -1"><span class="mobile-field-label">子文件任务</span><span class="mobile-field-value">{{ row.categoryNames || '未配置' }}</span></div>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑统筹任务' : '新建统筹任务'" width="880px" @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="104px">
        ...
        <el-form-item label="子文件任务" required>
          <div class="file-task-editor">
            <div class="file-task-editor-head">
              <div><strong>文件任务清单</strong><p>每个子任务对每个项目、每次提交只接收 1 个文件。</p></div>
              <el-button type="primary" plain @click="addFileTask"><el-icon><Plus /></el-icon>添加子任务</el-button>
            </div>
            <div v-for="(item, index) in form.fileTasks" :key="item.key" class="file-task-row">
              <div class="file-task-order">{{ index + 1 }}</div>
              <div class="file-task-fields">
                <el-input v-model.trim="item.categoryName" maxlength="120" placeholder="子任务名称，例如：项目申报书" />
                <el-input v-model.trim="item.taskDescription" maxlength="1000" placeholder="提交说明（可选）" />
              </div>
              <el-checkbox v-model="item.isRequired">必交</el-checkbox>
              <div class="file-task-actions">
                <el-button circle :disabled="index === 0" aria-label="上移" @click="moveFileTask(index, -1)"><el-icon><ArrowUp /></el-icon></el-button>
                <el-button circle :disabled="index === form.fileTasks.length - 1" aria-label="下移" @click="moveFileTask(index, 1)"><el-icon><ArrowDown /></el-icon></el-button>
                <el-button circle type="danger" plain :disabled="form.fileTasks.length === 1" aria-label="删除" @click="removeFileTask(index)"><el-icon><Delete /></el-icon></el-button>
              </div>
            </div>
          </div>
        </el-form-item>
...
let fileTaskKey = 0
const makeFileTask = (source = {}) => ({ key: ++fileTaskKey, id: source.id || null, categoryName: source.categoryName || '', taskDescription: source.taskDescription || '', isRequired: source.isRequired !== false })
const newForm = () => ({ id: null, taskName: '', taskDescription: '', projectScopeType: 'all', projectYear: new Date().getFullYear(), projectGroup: '', fileTasks: [makeFileTask()], deadlineAt: '', allowedExtensions: [...extensionOptions], maxFileMb: 50, maxTaskProjectMb: 500, hasTemplate: false })
...
function openEdit(row) {
  Object.assign(form, newForm(), row, {
    id: row.id,
    fileTasks: (row.fileTasks?.length ? row.fileTasks : String(row.categoryNames || '').split('、').filter(Boolean).map((categoryName) => ({ categoryName }))).map(makeFileTask),
...
function addFileTask() {
  form.fileTasks.push(makeFileTask())
}

function removeFileTask(index) {
  if (form.fileTasks.length > 1) form.fileTasks.splice(index, 1)
}

function moveFileTask(index, offset) {
  const target = index + offset
  if (target < 0 || target >= form.fileTasks.length) return
  const [item] = form.fileTasks.splice(index, 1)
  form.fileTasks.splice(target, 0, item)
}

function fileTasksPayload() {
  return form.fileTasks.map((item, sortOrder) => ({
    categoryName: item.categoryName.trim(), taskDescription: item.taskDescription.trim() || null,
    isRequired: item.isRequired, sortOrder,
  }))
}

async function saveTask(status) {
  if (!(await formRef.value?.validate().catch(() => false))) return
  if (!form.fileTasks.length || form.fileTasks.some((item) => !item.categoryName.trim())) return ElMessage.warning('请填写每个子文件任务的名称')
  const names = form.fileTasks.map((item) => item.categoryName.trim())
  if (new Set(names).size !== names.length) return ElMessage.warning('同一统筹任务下的子任务名称不能重复')
  saving.value = true
  try {
    ...
    const response = await apiRequest(form.id ? `/material-tasks/${form.id}` : '/material-tasks', {
      ...
      body: { ...form, status, fileTasks: fileTasksPayload(), hasTemplate: existingTemplates || (form.hasTemplate && templateFiles.value.length > 0) },
    })
