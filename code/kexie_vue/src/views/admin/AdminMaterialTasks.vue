  ...
  <div>
    ...
    <div class="panel">
      ...
      <el-table v-loading="loading" :data="tasks" class="desktop-table" style="width:100%" empty-text="暂无材料任务">
        <el-table-column label="统筹任务" min-width="180"><template #default="{ row }"><el-button text type="primary" class="task-name-link" @click="openEdit(row)">{{ row.taskName }}</el-button></template></el-table-column>
...
const newForm=()=>({id:null,editable:true,status:'draft',taskName:'',taskDescription:'',projectScopeType:'all',projectYear:new Date().getFullYear(),projectGroup:'',projectIds:[],fileTasks:[makeFileTask()],deadlineAt:'',maxTaskProjectMb:500})
...
function openEdit(row){Object.assign(form,newForm(),row,{id:row.id,editable:Boolean(row.editable),status:row.status,projectIds:[...(row.projectIds||[])],fileTasks:(row.fileTasks||[]).map(makeFileTask),deadlineAt:row.deadlineAt?String(row.deadlineAt).replace('T',' ').slice(0,19):''});dialogVisible.value=true}
