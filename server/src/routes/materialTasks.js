...
function taskPayload(body) {
  ...
  const categories = Array.isArray(body.categories)
    ? body.categories.map((item, index) => ({
    ...
    : [];
  if (!categories.length) throw badRequest('At least one material category is required', 'VALIDATION_ERROR');
...
router.get('/', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    const items = rows.map((row) => ({ ...row, scopeLabel: scopeLabel(row), hasTemplate: Boolean(row.hasTemplate) }));
  ...
  try {
    ...
    const [rows] = await pool.execute(
      `SELECT mt.id AS taskId, mt.task_name AS taskName, mt.task_description AS taskDescription,
              ...
              mc.id AS categoryId, mc.category_name AS categoryName, mc.is_required AS isRequired,
    ...
    for (const category of data.categories) {
      await connection.execute(
        `INSERT INTO material_categories (material_task_id, category_name, is_required, sort_order)
         VALUES (?, ?, ?, ?)`,
        [result.insertId, category.categoryName, category.isRequired ? 1 : 0, category.sortOrder]
      );
    }
...
router.put('/:id', requireAuth, requireRole('admin'), async (req, res, next) => {
  ...
  try {
    ...
    for (const category of data.categories) {
      await connection.execute(
        'INSERT INTO material_categories (material_task_id, category_name, is_required, sort_order) VALUES (?, ?, ?, ?)',
        [req.params.id, category.categoryName, category.isRequired ? 1 : 0, category.sortOrder]
      );
    }
    for (const projectId of data.projectIds) {
